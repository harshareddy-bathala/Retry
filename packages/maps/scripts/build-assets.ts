...
function buildFromPack(packRoot: string): Manifest {
  ...
  // Emotes: the pack ships them as a 10x10 GRID whose columns are animation
  // frames, so the chosen pairs are cropped into one single-row strip. The
  // typing bubble is appended last, after every pickable emote, which is what
  // lets the client address it as a fixed index without a lookup.
  const emoteSource = join(packRoot, EMOTE_SHEET);
  if (!existsSync(emoteSource)) throw new Error(`missing from the pack: ${EMOTE_SHEET}`);
  const emoteAtlas = decodePng(readFileSync(emoteSource));
  const emotePairs = [...EMOTES.map((e) => ({ col: e.col, row: e.row })), TYPING_FRAMES];
  const emoteStrip = new Canvas(emotePairs.length * 2 * TILE, TILE);
  emotePairs.forEach((pair, i) => {
    for (let frame = 0; frame < 2; frame++) {
      const cell = crop(emoteAtlas, (pair.col + frame) * TILE, pair.row * TILE, TILE, TILE);
      emoteStrip.blit(cell, (i * 2 + frame) * TILE, 0);
    }
  });
  writeFileSync(
    join(outRoot, 'emotes.png'),
    encodePng(emoteStrip.width, emoteStrip.height, emoteStrip.data),
  );
...
// Emotes: one strip, two frames each, in EMOTES order with the typing bubble
// last. The client needs the label (for the picker) and the frame offset; it
// must never need to know the atlas coordinates the crop came from.
const hasEmotes = manifest.source === 'limezu';
writeFileSync(
  join(outRoot, 'emotes.ts'),
  [
    '// GENERATED by scripts/build-assets.ts — do not edit, do not commit.',
    hasEmotes ? "import url from './emotes.png';" : 'const url = null;',
    '',
    ...
    `export const EMOTE_FRAME_SIZE = ${TILE};`,
    '',
    'export const EMOTE_CHOICES: EmoteChoice[] = [',
    ...(hasEmotes
      ? EMOTES.map(
          (e, i) =>
            `  { key: ${JSON.stringify(e.key)}, label: ${JSON.stringify(e.label)}, frame: ${i * 2} },`,
        )
      : []),
    '];',
    '',
    '/** First frame of the three-dot "is typing" bubble. */',
    `export const TYPING_FRAME = ${EMOTES.length * 2};`,
    '',
    '/** Every key a client may send. Validated server-side against the same list. */',
    `export const EMOTE_KEYS: string[] = [${EMOTES.map((e) => JSON.stringify(e.key)).join(', ')}];`,
    '',
  ].join('\n'),
);

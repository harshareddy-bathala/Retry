...
| Tailwind CSS | v4 | Utility-first; no CSS files per component; design tokens in `tailwind.config`. |
| Radix UI | latest | Unstyled accessible primitives (Dialog, Dropdown, Tabs, Toast). Styled with Tailwind. |
| Framer Motion | 11.x | Micro-interactions and page transitions only. No layout-critical animation. |
...
| @fastify/websocket (ws) | latest | Room server: presence, avatar positions, chat, Kanban, Blueprint sync. |
...
| Daily.co (REST API) | — | WebRTC audio/video. Server orchestrates room tokens + proximity session membership. No custom WebRTC. |
...
| Redis | 7 | Feed ranking cache (15-min recompute), BullMQ backing store, WebSocket pub/sub for horizontal room-server scaling. |
...
| Process manager | systemd units | api, room server (same process, separate plugin scope), grading worker, tldraw sync | — |
...
| Daily.co | Proximity audio/video in Live Space | Free tier; monitor participant-minutes (admin dashboard duty) |
...
- **No Socket.io** — plain `ws` via Fastify plugin; we control the event protocol (`WEBSOCKET_EVENTS.md`).
- **No custom WebRTC** — Daily.co only (FR-ROOM-29).

...
| `retry-api` (REST + room WS) | Droplet, systemd, port 3000 |
| `retry-worker` (BullMQ consumers) | Droplet, systemd |
| `retry-tldraw` (whiteboard sync) | Droplet, systemd, port 3001 |
...
Each unit: `Restart=always`, `RestartSec=3`, memory limits (`api` 1.5G, `worker` 1G, `tldraw` 512M), `EnvironmentFile=/etc/retry/*.env`, logs to journald (shipped to New Relic).
